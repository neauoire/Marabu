...
function Sequencer (terminal) {
  ...
  this.drawChannels = function () {
    ...
    for (var channel = 0; channel < 16; channel++) {
      ...
      const loop = terminal.track.getLoop(channel, terminal.cursor.pos.t)
      html += `<div class='channel ${isSelected ? 'sel' : ''}'><span class='name ${isSelected ? 'sel' : ''}'>${channel.toString(16).toUpperCase()}${loop.toString(16).toUpperCase()}</span>${this.drawNotes(channel)}</div>`
    }
    return html
  }

  this.drawNotes = function (channel) {
    terminal.cursor.getLoop()
    let html = ''
